...
```{r}
linear<-lm(model_formula, data=train) #fit linear model
lambda<-cv.glmnet(x=predictor_matrix_train, y=train$log_price_doc, alpha=1)$lambda.min #find min lambda
lasso<-glmnet(x=predictor_matrix_train, y=train$log_price_doc, alpha=1, lambda=lambda) #fit lasso with lambda star

test$pred_lin<-exp(predict(linear, test)) #predict with lin model
test$pred_lasso<-exp(predict(lasso, predictor_matrix_test)) #predict with lasso model 

train$pred_lin<-exp(predict(linear, train)) #predict on training set
train$pred_lasso<-exp(predict(lasso, predictor_matrix_train)) #predict on training set

sub_linear<-test[,c(1,293)] #create submission linear
sub_lasso<-test[,c(1,294)]  #create submission lasso
colnames(sub_linear)[2]="price_doc"
colnames(sub_lasso)[2]="price_doc"

write.csv(sub_linear, "linear_sub.csv", row.names = FALSE)
write.csv(sub_lasso, "lasso_sub.csv", row.names=FALSE)

train$pred_lin<-exp(predict(linear, train)) 
train$pred_lasso<-exp(predict(lasso, predictor_matrix_train))

rmsle_train_lin=rmsle(train$price_doc, train$pred_lin) #rmsle train with linear
rmsle_train_lin

rmsle_train_lasso=rmsle(train$price_doc, train$pred_lasso) #rmsle train with lasso
rmsle_train_lasso
```
...
LASSO crossvalidated  |     0.4544997   |  0.41917
